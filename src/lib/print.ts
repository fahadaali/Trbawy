// توليد صفحة المحضر القابلة للطباعة/التصدير (HTML عربي RTL مع الهوية البصرية ورمز QR).
...
export async function renderMeetingHtml(env: Env, meetingId: number, origin: string): Promise<string | null> {
  ...
  ).bind(meetingId).all()).results as any[];

  const agenda = (await env.DB.prepare(
    'SELECT * FROM agenda_items WHERE meeting_id = ? ORDER BY sort_order',
  ).bind(meetingId).all()).results as any[];

  const actions = (await env.DB.prepare(
    'SELECT * FROM action_items WHERE source_meeting_id = ? ORDER BY id',
  ).bind(meetingId).all()).results as any[];

  const writer = m.writer_id
    ? await env.DB.prepare('SELECT name FROM users WHERE id = ?').bind(m.writer_id).first<any>() : null;
  const approver = m.approved_by
    ? await env.DB.prepare('SELECT name FROM users WHERE id = ?').bind(m.approved_by).first<any>() : null;
  ...
  const watermark = settings.watermark_key
    ? `<div class="watermark"><img src="/file?key=${encodeURIComponent(settings.watermark_key)}" /></div>` : '';

  const signRows = attendees.filter((a) => !a.is_guest).map((a) => {
    let sig = '—';
    if (a.signed_at) {
      sig = a.signature_image
        ? `<img class="sig" src="/file?key=${encodeURIComponent(a.signature_image)}" />`
        : `<span class="stamp">${esc(a.user_name)}</span>`;
    } else if (a.signature_override) {
      sig = '<span class="ov">تجاوز موثّق</span>';
    } else if (a.attendance_status !== 'present') {
      sig = `<span class="muted">${esc(ATT_AR[a.attendance_status])}</span>`;
    }
    return `<tr><td>${esc(a.user_name)}</td><td>${esc(ATT_AR[a.attendance_status] || '')}</td>
      <td>${sig}</td><td class="code">${a.signature_hash ? esc(a.signature_hash) : ''}</td>
      <td>${a.signed_at ? esc(a.signed_at) : ''}</td></tr>`;
  }).join('');

  const guests = attendees.filter((a) => a.is_guest);

  return `<!DOCTYPE html><html lang="ar" dir="rtl"><head><meta charset="UTF-8" />
<title>${esc(m.display_number)}</title>
...
<div class="page-foot"><span>${esc(settings.footer_text || '')}</span><span dir="ltr">${esc(m.display_number)}</span></div>
<div class="content">
  <h1>محضر اجتماع ${m.status === 'approved' || m.status === 'archived' ? '' : '(مسودة)'}</h1>
  <div class="subnum">${esc(m.display_number)}</div>
  ${m.title ? `<p style="text-align:center">${esc(m.title)}</p>` : ''}
  ${m.status === 'approved' || m.status === 'archived' ? `<div class="approved-badge">✔ محضر معتمد ومقفل${approver ? ' — اعتمده: ' + esc(approver.name) : ''}</div>` : ''}
  ...
  <ol class="agenda">${agenda.map((it) => `<li><b>${esc(it.title)}</b>${it.body ? `<div>${esc(it.body)}</div>` : ''}</li>`).join('') || '<li>—</li>'}</ol>
...
export async function renderVerifyHtml(env: Env, code: string): Promise<string> {
  ...
  if (!m) {
    return head + `<h1>التحقق من المحضر</h1><p class="bad">لا يوجد محضر بهذا الرمز. قد يكون الرمز غير صحيح.</p></div></body></html>`;
  }

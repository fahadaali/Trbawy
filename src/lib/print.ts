...
// (كانا position: fixed: Chrome يكرّرهما على كل صفحة، وSafari يرسمهما مرة واحدة في وسط
...
function printStyles(primary: string, font: string): string {
  ...
  * { box-sizing: border-box; }
  ...
  /* إطار الصفحة: الترويسة في thead والتذييل في tfoot — فيتكرّران على كل صفحة
     ...
     (كانا عنصرين ثابتين position:fixed: Chrome يكرّرهما، وSafari يرسمهما مرة واحدة
...
(function () {
  ...
  // المشاركة الأصلية: أسرع طريق على الجوال لإرسال المحضر أو فتحه في Safari للطباعة
  if (navigator.share) {
  ...
  // في وضع التطبيق المستقل على iPhone قد لا تتوفّر نافذة الطباعة — نرشد للطريق البديل
  if (ios) {
    document.getElementById('printHint').textContent = standalone
      ? 'إن لم تفتح نافذة الطباعة: اضغط «مشاركة الرابط» ثم افتحه في Safari واختر «طباعة» أو «حفظ في الملفات».'
